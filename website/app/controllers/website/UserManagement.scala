...
object UserManagement extends Controller {
   ...
   */
  def logout = Action {
    Redirect(routes.UserManagement.login).withNewSession.flashing(
...
trait Secured {
   ...
   */
  private def onUnauthorized(request: RequestHeader) = Results.Redirect(routes.UserManagement.login)
